...
def verify_email(db: Session, email: str, verification_code: str) -> Optional[models.User]:
    try:
        ...
        if db_user.email_verification_code_expires_at < datetime.now(timezone.utc):
            ...
            # db.commit()
            return None # Indicate code expired
...
def reset_password(db: Session, email: str, verification_code: str, new_password: str) -> Optional[models.User]:
    try:
        user_query = db.query(models.User).filter(
            models.User.email == email,
            models.User.email_verification_code == verification_code
...
def get_user_stats(db: Session, user_id: int) -> Dict[str, Any]:
    try:
        ...
        xp_points = user.xp_points if user else 0
        ...
        # Return a default structure on error to avoid breaking API response schemas
        return {
            "total_lessons": 0,
            "completed_lessons": 0,
            ...
            "xp_points": 0
        } # Ensure this matches expected schema if endpoint directly uses it. 